...
ID-token verification, protected-route middleware, and the /api/me contract.

## Runtime model

createAuth returns handle, getUser, and middleware. The middleware owns reserved
auth routes, applies same-origin protection to mutations, loads the current
session, and either short-circuits or calls the next site handler.
